data "http" "myip" {
  count = var.db_instance_with_public_ip ? 1 : 0
  url   = "http://ipv4.icanhazip.com"
}

data "google_compute_network" "recipy_vpc" {
  name = data.terraform_remote_state.network.outputs.recipy_vpc_name
}
...
resource "google_sql_database_instance" "recipy_instance" {
  ...
  settings {
    ...
    ip_configuration {
      ...
      private_network = data.google_compute_network.recipy_vpc.id
